                ...
                if go_all_out:
                    if game_state.turn_number % 8 == 1:
                        ...
                        if self.pick_spawn_point_side(game_state, unit_type=None, valid_x=None, valid_y=[14, 27]) == 0:
                            game_state.attempt_spawn(EMP, the_spawn_location_options[0], 100)
                        else:
                            #spawn right side
                            game_state.attempt_spawn(EMP, the_spawn_location_options[1], 100)
                    else:
                        ...
                        if self.pick_spawn_point_side(game_state, unit_type=None, valid_x=None, valid_y=[14, 27]) == 0:
                            ...
                            game_state.attempt_spawn(PING, the_spawn_location_options[0], 4)
                        else:
                            #spawn right side
                            # Sending more at once is better since attacks can only hit a single ping at a time
    ...
    def build_reactive_defense(self, game_state):
        ...
        for location in self.scored_on_locations:
            ...
            if build_location in left:
                ...
                game_state.attempt_upgrade(destructor_left_locations)
                #print(coord)
            if build_location in right:
#                coord = random.choices(self.destructor_right_locations,k=1)
                #print(coord)
                game_state.attempt_upgrade(destructor_right_locations)
            else:
